...
export function bloomBitArrayBytes(m: number): number {
 ...
 */
export function estimatedHashSetBytes(strings: readonly string[]): number {
  let payload = 0
...
export interface BloomVsHashSetComparison {
  bloomBytes: number
  hashSetBytes: number
  savedBytes: number
...
export function compareBloomVsHashSet(
  m: number,
  strings: readonly string[],
): BloomVsHashSetComparison {
